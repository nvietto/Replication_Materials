...
data <- read.csv("data2.csv") # Data mean imputed to compensate for 0s in autonomic measures 
...
Within the HR measures for each stress paradigm, a small number of individuals had zero values in random columns (typically 1-4 columns or seconds). These zeros likely resulted from hand movements during the task. To retain these cases in the analysis, we replaced the zero values with the mean of the values from the second before and the second after the zero value.
